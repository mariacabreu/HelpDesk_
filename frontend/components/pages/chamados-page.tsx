...
export function ChamadosPage() {
  ...
  const fetchChamados = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      let url = user.role === "suporte" 
        ? `/api/chamados/escalados-disponiveis/${user.nivel || 'n1'}`
        : "/api/chamados"
      
      // Se for empresa, filtrar por empresa_id
      if (user.role === "empresa" && user.empresa?.id) {
        url = `/api/chamados/empresa/${user.empresa.id}`
      }
  ...
  useEffect(() => {
    ...
    fetchChamados(controller.signal)
  ...
  const chamadosFiltrados = chamados.filter(c => {
    // Garantir que apenas chamados escalonados sejam exibidos nesta página
    const statusEscalonados = ["escalado", "escalonamento_aprovado"];
    if (!statusEscalonados.includes(c.status)) return false;
            ...
            <div className="flex flex-col gap-2">
              ...
              <Select value={filtros.status} onValueChange={(value) => setFiltros({ ...filtros, status: value })}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="escalonamento_aprovado">Escalonamento Aprovado</SelectItem>
                </SelectContent>
              </Select>
            </div>
