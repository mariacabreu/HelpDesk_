...
export function DashboardEmpresaPage() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      if (empresaId) {
        // Buscar estatísticas
        fetch(`/api/stats/empresa/${empresaId}`, { signal: controller.signal })
        ...
        fetch(`/api/chamados/empresa/${empresaId}`, { signal: controller.signal })
          .then(res => safeJson<any[]>(res))
          .then(data => {
            if (!data) return
          ...
          .catch(err => {
            if (err.name !== 'AbortError') {
              console.error("Erro ao buscar chamados recentes:", err)
            }
          })
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-muted-foreground">
            {empresa?.nome_fantasia || "Minha Empresa"} - Gestão de chamados
          </p>
        </div>
