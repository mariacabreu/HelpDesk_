...
export function EscalonadosPage() {
  ...
  const fetchEscalonados = async (signal?: AbortSignal) => {
    ...
    try {
      ...
      let url = `/api/chamados/atribuido/${user.id}`
      
      // Se for empresa, buscar por empresa_id (ajustado para visualização correta)
  ...
  useEffect(() => {
    ...
    fetchEscalonados(controller.signal)
