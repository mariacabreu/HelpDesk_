...
export function NovoChamadoPage({ onTicketCreated }: NovoChamadoPageProps) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserData(user)
      const adminStatus = ["Empresa", "Diretor", "Desenvolvedora"].includes(user.cargo)
      ...
      if (user.empresa?.id) {
        ...
        // Buscar todos os funcionários da empresa para filtrar por nível depois (apenas para admins)
        if (adminStatus) {
