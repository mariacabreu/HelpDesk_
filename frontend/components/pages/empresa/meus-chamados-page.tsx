...
export function MeusChamadosPage() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      if (solicitanteId) {
        // Se for administrador da empresa, ver todos os chamados da empresa
        const isAdmin = ["Empresa", "Diretor", "Desenvolvedora"].includes(user.cargo)
        const url = isAdmin && user.empresa?.id 
          ? `/api/chamados/empresa/${user.empresa.id}`
          : `/api/chamados/solicitante/${solicitanteId}`
      ...
      if (empresaId) {
        fetch(`/api/equipamentos/${empresaId}`, { signal: controller.signal })
          .then(res => safeJson<any[]>(res))
          .then((data) => {
