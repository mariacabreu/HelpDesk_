...
export function CompanyForm({ onBack }: CompanyFormProps) {
  ...
  const handleChange = async (id: string, value: string) => {
    ...
    // Atualiza o valor do campo imediatamente
    setFormData((prev) => ({ ...prev, [id]: maskedValue }))

    // Preenchimento automático ao digitar 8 dígitos no CEP
    if (id === "cep" && maskedValue.replace(/\D/g, "").length === 8) {
  ...
  const validate = () => {
    ...
    if (formData.senhaEmpresa.length > 72) {
      newErrors.senhaEmpresa = "A senha não pode ter mais de 72 caracteres"
    }
