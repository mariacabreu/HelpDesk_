...
# Rota de compatibilidade para deploy (alguns proxies adicionam /api no prefixo)
@app.api_route("/api", methods=["GET", "HEAD"])
def read_api_root():
    return read_root()

# Middleware para lidar com prefixo /api e log de requisições
@app.middleware("http")
async def api_middleware(request: Request, call_next):
    # Log básico para depuração no Render
    print(f"DEBUG: {request.method} {request.url.path}")
    
    # Tratar prefixo /api/
    if request.url.path.startswith("/api/"):
        new_path = request.url.path.replace("/api/", "/", 1)
...
def buscar_cep(cep: str):
    ...
    try:
        # Adicionando timeout e cabeçalho User-Agent para evitar bloqueios
        response = requests.get(url, timeout=10, headers={"User-Agent": "SwiftDesk/1.0"})
        print(f"Resposta ViaCEP: {response.status_code}")
        ...
        print(f"Erro ao consultar ViaCEP: {str(e)}")
...
# As classes de modelo Pydantic e middlewares já foram definidas acima.
# Removendo middleware de CORS duplicado.
...
def create_empresa(empresa: EmpresaCreate, db: Session = Depends(get_db)):
    print(f"Iniciando criação de empresa: {empresa.nome_fantasia}")
    try:
        ...
        db.add(db_empresa)
        db.flush() # Flush para garantir que o id seja gerado sem commitar a transação inteira ainda

        # Criar o funcionário administrador para a empresa
        # Truncar senha para 72 bytes para evitar erro do bcrypt/database se necessário
        senha_segura = empresa.senha[:72] if empresa.senha else "123456"

        db_funcionario = Funcionario(
            empresa_id=db_empresa.id,
            nome=empresa.nome_responsavel or empresa.nome_fantasia or "Administrador",
            email=empresa.email,
            telefone=empresa.telefone,
        ...
        print(f"ERRO CRÍTICO AO CRIAR EMPRESA:\n{error_msg}")
...
def get_escalados_disponiveis(nivel: str, db: Session = Depends(get_db)):
    ...
    # Query base: chamados sem técnico e que estão com status de escalonamento
    query = db.query(Chamado).filter(
        Chamado.atribuido_a_id == None,
        Chamado.status.in_(["escalado", "escalonamento_aprovado"])
    )
    ...
    if nivel == "n2":
        ...
        chamados = query.filter(Chamado.escalonado_por_nivel.in_(["n2", "n3"])).all()
    else:
        # N1 não vê chamados escalonados (pois ele é o nível base)
        chamados = []
...
def get_stats_empresa(empresa_id: int, db: Session = Depends(get_db)):
    ...
    # Funcionários ativos da empresa (excluindo a conta master da empresa e cargos administrativos)
    total_funcionarios = db.query(func.count(Funcionario.id)).filter(
        Funcionario.empresa_id == empresa_id,
        Funcionario.status == "ativo",
        func.lower(Funcionario.cargo).notin_(["empresa", "diretor", "desenvolvedora"]),
        func.lower(Funcionario.permissao) != "empresa"
    ).scalar()
